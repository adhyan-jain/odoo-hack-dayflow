...
| **Notifications** | `lib/email/mailer.ts` sends real transactional emails over Brevo's HTTP API for employee welcome, leave submitted, and leave approved/rejected (Next.js API routes); the `send-notification` Edge Function sends the pg_cron leave-escalation email the same way. Both fall back to `console.log` if `BREVO_API_KEY`/`BREVO_SENDER_EMAIL` are unset (zero-config demo path) | Delivery tracking/retries, richer HTML templates, and a deployed (not just written) Edge Function in every environment |
