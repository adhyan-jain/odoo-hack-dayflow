...
| **Notifications** | `send-notification` Edge Function either `console.log`s the payload (default, zero-config demo path) or sends via Resend's free tier if `RESEND_API_KEY` is set | Proper transactional email/SMS/push provider with delivery tracking, retries, and templates |
